...
def release_asset_paths(artifact_path: Path, manifest: dict[str, Any]) -> list[Path]:
    ...
    container_descriptor = "payloads/containers/loopaware/container.json"
    assets = [artifact_path / "manifest.json"] + [
        artifact_path / entry["path"]
        for entry in manifest.get("payloads", [])
        if entry["path"].startswith(prefix) or entry["path"] == container_descriptor
    ]
    publication_attestation = artifact_path / "publication.json"
    if publication_attestation.is_file():
        assets.append(publication_attestation)
...
def release_asset_plan(cwd: Path, version: str, assets: list[Path]) -> list[Path]:
    ...
    if set(existing_by_name) - set(expected_by_name):
        raise HelperError(
...
def command_publish_prepared_release(args: argparse.Namespace) -> int:
    ...
    if not re.fullmatch(r"v(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)", version):
        fail("prepared release version must be stable vMAJOR.MINOR.PATCH", {"version": version})
