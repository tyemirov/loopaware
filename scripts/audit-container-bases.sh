...
build_base='FROM golang:1.27.1-alpine3.24@sha256:cf6fca6641884b8433441b2b0652976f975e1d0fdd26d177eaaf8596087f3125 AS build'
...
if ! grep -Fqx "${build_base}" "${dockerfile}"; then
  echo "container_base_audit_failed: Dockerfile build base must use the approved multi-architecture Go 1.27.1/Alpine 3.24 digest" >&2
  exit 1
fi
