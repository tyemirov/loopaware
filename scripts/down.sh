...
usage() {
  cat <<'USAGE'
Usage: ./scripts/down.sh [local|computercat]
...
resolve_stack_target() {
  ...
  if [[ ! -t 0 ]]; then
    echo "error: non-tty runs are not supported. Re-run ./scripts/down.sh in an interactive terminal." >&2
    exit 1
  fi
...
case "${mode}" in
  ...
  computercat)
    ...
    ;;
  *)
    echo "Usage: ./scripts/down.sh [local|computercat]" >&2
    exit 1
    ;;
esac
