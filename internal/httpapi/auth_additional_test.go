...
func newAuthManager(testingT *testing.T, databaseConfig *storage.Config) *AuthManager {
	...
	manager, createErr := NewAuthManager(databaseHandle, zap.NewNop(), nil, nil, testAuthRedirectPath, AuthConfig{
...
func TestRequireAuthenticatedWebRedirectsWhenMissingSession(testingT *testing.T) {
	gin.SetMode(gin.TestMode)
	authManager := &AuthManager{
		logger:                      zap.NewNop(),
		unauthenticatedRedirectPath: testAuthRedirectPath,
	}
	recorder := httptest.NewRecorder()
	ginContext, _ := gin.CreateTestContext(recorder)
	ginContext.Request = httptest.NewRequest(http.MethodGet, "/app", nil)

	handler := authManager.RequireAuthenticatedWeb()
	handler(ginContext)

	require.Equal(testingT, http.StatusFound, recorder.Code)
	require.Equal(testingT, testAuthRedirectPath, recorder.Header().Get("Location"))
}
...
func TestRequireAuthenticatedWebAllowsWhenUserPresent(testingT *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/", nil)
	...
	authManager := &AuthManager{logger: zap.NewNop(), unauthenticatedRedirectPath: "/login"}
	handler := authManager.RequireAuthenticatedWeb()
	handler(context)
	require.False(testingT, context.IsAborted())
}
...
func TestNewAuthManagerDefaultsDependencies(testingT *testing.T) {
	manager, managerErr := NewAuthManager(nil, zap.NewNop(), nil, nil, "", AuthConfig{SigningKey: "signing-key"})
	require.NoError(testingT, managerErr)
	require.NotNil(testingT, manager.httpClient)
	require.Equal(testingT, LandingPagePath, manager.unauthenticatedRedirectPath)
}

func TestNewAuthManagerSkipsBlankAdminEmail(testingT *testing.T) {
	manager, managerErr := NewAuthManager(nil, zap.NewNop(), []string{testAuthBlankEmail, testAuthUserEmail}, nil, testAuthRedirectPath, AuthConfig{SigningKey: testAuthSigningKeyValue})
	require.NoError(testingT, managerErr)
