...
const (
	...
	dashboardFooterLayoutToken                 = "mpr-footer__layout"
	dashboardFooterBrandWrapperToken           = "mpr-footer__brand"
	dashboardFooterPrivacyClassToken           = "mpr-footer__privacy"
	...
	dashboardLogoutHelperToken                 = "typeof window.logout === 'function'"
	dashboardLogoutPromiseToken                = "Promise.resolve(window.logout())"
	...
	dashboardFooterDropdownToggleToken         = "mpr-footer__menu-button"
	dashboardFooterDropdownMenuToken           = "mpr-footer__menu"
	...
	dashboardWidgetBottomOffsetInputClassToken = "class=\"form-control text-end widget-bottom-offset-input\""
)
...
func TestDashboardPageRendersForAuthenticatedUser(t *testing.T) {
	...
	handlers := httpapi.NewDashboardWebHandlers(zap.NewNop(), testDashboardLandingPathRoot, testLandingAuthConfig)
...
func TestDashboardTemplateUsesSitesListPanel(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			testName:      "logout uses tauth helper",
			substring:     dashboardLogoutHelperToken,
			expectPresent: true,
		},
		{
			testName:      "logout helper resolves promise",
			substring:     dashboardLogoutPromiseToken,
			expectPresent: true,
		},
		{
...
func TestDashboardWidgetBottomOffsetInputUsesCompactWidth(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/app", nil)
	...
	body := recorder.Body.String()
	require.Contains(t, body, dashboardWidgetBottomOffsetInputClassToken)
}
