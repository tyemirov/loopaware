...
func TestWidgetIntegrationRecordsNoDeliveryWhenNotifierFails(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, apiHarness.database.Find(&stored).Error)
	require.Len(t, stored, 1)
	require.Equal(t, model.FeedbackDeliveryNone, stored[0].Delivery)
}

func TestWidgetIntegrationKeepsDeliveryNoneWhenNotifierReturnsStatusAndError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	call := notifier.LastCall()
	require.Equal(t, site.ID, call.Site.ID)
	require.Equal(t, integrationFeedbackContactValue, call.Feedback.Contact)
