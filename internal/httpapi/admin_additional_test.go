...
func TestCreateSiteRejectsInvalidOwner(testingT *testing.T) {
	...
	recorder, context := newJSONContext(http.MethodPost, "/api/sites", payload)
	context.Set(testSessionContextKey, &httpapi.CurrentUser{Email: "   ", Role: httpapi.RoleAdmin})
...
func TestUserAvatarReportsQueryError(testingT *testing.T) {
	...
	context.Set(testSessionContextKey, &httpapi.CurrentUser{Email: testUserEmailAddress, Role: httpapi.RoleAdmin})
