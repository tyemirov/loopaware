...
var testStreamAuthConfig = NewAuthClientConfig("stream-client", "https://tauth.example", "stream-tenant")
...
func TestStreamSubscriptionTestEventsWritesEvent(testingT *testing.T) {
	...
	handlers := NewSiteSubscribeTestHandlers(database, zap.NewNop(), eventBroadcaster, nil, true, testStreamPublicBaseURL, testStreamSubscriptionSecret, nil, testStreamAuthConfig)
