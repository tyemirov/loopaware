...
func buildSubscribeHandlers(testingT *testing.T) *SiteSubscribeTestHandlers {
	...
	handlers := NewSiteSubscribeTestHandlers(database, zap.NewNop(), broadcaster, nil, false, testSubscribeBaseURL, testSubscribeTokenSecret, nil)
	return handlers
}
...
func insertSubscribeSite(testingT *testing.T, handlers *SiteSubscribeTestHandlers) {
	...
	site := model.Site{
		...
		CreatorEmail:  testSubscribeOwnerEmail,
	}
