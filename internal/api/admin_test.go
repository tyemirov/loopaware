...
const (
	...
	testSessionContextKey               = "api_current_user"
...
func newSiteTestHarness(testingT *testing.T) siteTestHarness {
	...
	handlers := api.NewSiteHandlers(database, zap.NewNop(), testWidgetBaseURL, nil, nil, nil)
...
func TestCurrentUserReturnsAvatarPayload(testingT *testing.T) {
	...
	context.Set(testSessionContextKey, &api.CurrentUser{
	...
	require.Equal(testingT, string(api.RoleUser), payload.Role)
...
func TestListSitesReturnsOwnerSitesForNonAdminRegardlessOfEmailCase(testingT *testing.T) {
	...
	recorder, context := newJSONContext(http.MethodGet, "/api/sites", nil)
	context.Set(testSessionContextKey, &api.CurrentUser{Email: strings.ToLower(ownerEmail), Role: api.RoleUser})
...
func TestListSitesReturnsCreatorSitesForNonAdmin(testingT *testing.T) {
	...
	recorder, context := newJSONContext(http.MethodGet, "/api/sites", nil)
	context.Set(testSessionContextKey, &api.CurrentUser{Email: testCreatorEmail, Role: api.RoleUser})
...
func TestStreamFaviconUpdatesEmitsEvents(testingT *testing.T) {
	...
	faviconManager := api.NewSiteFaviconManager(
		database,
		service,
		zap.NewNop(),
	...
	handlers := api.NewSiteHandlers(database, zap.NewNop(), testWidgetBaseURL, faviconManager, nil, nil)
...
func TestStreamFeedbackUpdatesReceivesCreateEvents(testingT *testing.T) {
	...
	publicHandlers := api.NewPublicHandlers(database, zap.NewNop(), feedbackBroadcaster, subscriptionEvents, nil, nil, true, testWidgetBaseURL, "unit-test-session-secret", nil)
...
func TestUserAvatarReturnsStoredImage(testingT *testing.T) {
	...
	context.Set(testSessionContextKey, &api.CurrentUser{Email: testUserEmailAddress})
...
func TestUserAvatarReturnsNotFoundForEmptyEmail(testingT *testing.T) {
	...
	recorder, context := newJSONContext(http.MethodGet, "/api/me/avatar", nil)
	context.Set(testSessionContextKey, &api.CurrentUser{Email: " ", Role: api.RoleUser})
...
func TestListSubscribersReturnsDataForAdmin(testingT *testing.T) {
	...
	var response api.SiteSubscribersResponse
