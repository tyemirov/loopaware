...
`<mpr-footer>` does not touch authentication; LoopAware can drop it in today as long as we load `mpr-ui.css` first, then `mpr-ui.js` from the published CDN URLs (the bundle auto-registers the custom elements). No Alpine wiring or `mprFooter()` factory is required on `mpr-ui` v0.2.0+.
