...
- New endpoint `POST /public/subscriptions` handled by `PublicHandlers`:
  - Payload: `{ "site_id": "...", "email": "...", "name": "...", "source_url": "...", "audience_key": "..." }`.
  ...
  - Persist every new or resubscribed record with `Status=pending` until its signed confirmation link is used.
...
- Signed confirmation flow:
  - Every public subscription starts as `pending` and receives a short-lived HMAC token scoped to subscriber ID, site ID, and email.
  - The email opens `/subscriptions/confirm?token=...`; that page calls `GET /public/subscriptions/confirm-link?token=...` to set the subscriber to `confirmed`.
  - The confirmation UI exposes `/subscriptions/unsubscribe?token=...`; that page calls `GET /public/subscriptions/unsubscribe-link?token=...` to set the subscriber to `unsubscribed`.
  - Public callers cannot query or mutate subscription state with an email address and site ID.
...
- New authenticated endpoints under `/api/sites/:id/subscribers` (owner/admin):
  ...
  - `GET /export` to download subscriber rows as CSV.
...
- Serve `/subscribe.js?site_id=...` from `web/subscribe.js` (static asset, ESM-friendly with `@ts-check`).
  - Renders a semantic `<form>` with email and optional name fields; inline mode is the default, with an optional floating button variant via `mode=bubble`.
...
### Migration & rollout
- Keep the sample runtime configuration aligned with confirmation-email delivery and subscription-notification settings.
