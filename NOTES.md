...
### BugFixes
    ...
    - [ ] [LA-89] Restore the them toggle when user logs into dashboard. There is currently no theme toggle and for no reason the theme selected is light. Trace the logic of theme identification and ensure no bugs. Selector to the right means dar theme, selector to the left means light theme
    - [ ] [LA-90] The test page is unstyled (and unusable). Ensure thjat we have the same header and footer in the test page with the theme switch, logoff etc
    ```
