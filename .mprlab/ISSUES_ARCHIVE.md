...
Historical completed issue records migrated from the legacy `issues.md/` workflow and archived from `.mprlab/ISSUES.md`. `.mprlab/ISSUES.md` remains the current live backlog for active, blocked, planning, and recurring work.

Archived batches preserve the IDs they held in their source tracker snapshot. Resolved dependency references from the active tracker may point here, and identical IDs can appear across older archived eras that predate the current live numbering.

## Archived from `.mprlab/ISSUES.md` on 2026-08-09

Last run: M400R archived 120 closed non-recurring BugFixes, Improvements, and Features entries so `.mprlab/ISSUES.md` stays focused on 20 unresolved, blocked, or recurring items. Open duplicate I027 remains cross-referenced to I028 in the active tracker; no active, blocked, planning, or recurring work was archived.
...
- [x] [B078] (P0) Resolve the CodeQL security findings exposed by default setup.
  ...
  Hosted Stack Validation Follow-up (2026-08-05):
  The repaired dependency stack exposed a browser-test isolation gap on every hosted branch: the four-hour auth scenario completed its Google/TAuth exchange and rendered the authenticated dashboard, but its console-clean assertion also captured connection resets from three unrelated page-view requests to the production LoopAware analytics endpoint. The shared external-asset fixture now fulfills only that exact production `/public/visits` pixel boundary with a transparent GIF, keeping the auth regression strict while preventing tests from writing to or depending on production analytics. `make lint-js` and the focused 100-scenario browser-security gate passed, including the previously failing stale-idle login scenario. The exact final `make ci` then passed every audit, vulnerability scan, build, lint/type check, Go test and race test, plus all 456 browser/API scenarios in 4.4 minutes.

  Changed Files:
  `PLAN.md`, `.mprlab/ISSUES.md`, `README.md`, `clients/react-native/src/index.tsx`, `internal/api/middleware.go`, `internal/api/middleware_test.go`, `mobile/scripts/fix-ios-project-warnings.mjs`, `mobile/scripts/validate-mobile-config.mjs`, `tests/README.md`, `tests/helpers/config.js`, `tests/helpers/externalAssets.js`, `tests/helpers/fixtures.js`, `tests/specs/api-public.spec.js`, `tests/specs/header-auth-state.spec.js`, and `web/app/index.html`; GitHub alerts 9 and 10 also received evidence-specific false-positive dispositions before the direct test request-header flow was removed.
...
## Items Migrated From Legacy issues.md/ISSUES.md
