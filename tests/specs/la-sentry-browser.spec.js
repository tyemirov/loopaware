...
test('browser LA Sentry harness captures explicit errors', async ({ page }) => {
  ...
  await page.goto(`/la-sentry-integration/?${search.toString()}`, { waitUntil: 'domcontentloaded' });
  await expect(page.locator('#la-sentry-integration-status')).toContainText('Loaded');

  const capture = await page.evaluate(async (message) => {
    const sentryClient = /** @type {any} */ (window).LASentry;
    if (!sentryClient || typeof sentryClient.captureError !== 'function') {
      throw new Error('missing LA Sentry browser client');
    }
  ...
  if (!issue) {
    throw new Error('browser LA Sentry issue was not created');
  }
  ...
  expect(detail.latest_occurrence.request.url).toBe(`${config.baseOrigin}/la-sentry-integration/`);
});
